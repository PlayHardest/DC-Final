...
Counters
	...
	-(DONE)evasive|mastery: dodge away in any cardinal direction + the air while having the ability to cancel the dodge into an attack (cancelling the dodge into the air into an
						attack will trigger an attack which launches all opponents hit, any other direction will knock away the enemy, not combo-able)
...
Pursue
	-(DONE)Auto tracking attacks cannot be carried out on someone using hyper movement
	-(DONE)Normal method, chase mechanic to approach an enemy, direction can be influenced
